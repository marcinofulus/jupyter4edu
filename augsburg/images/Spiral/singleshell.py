...
def put_label(x, y, nr):
    ...
    if 10 <= nr <= 25:
        bgcolor = color.rgb(0.6, 0.1, 0)
    if nr == 9:
        bgcolor = color.rgb(0, 0.1, 0.6)
    if abs(x) == abs(y):
        c.fill(path.circle(x, y, r), [bgcolor])
        c.text(x, y, label, [text.halign.center, text.valign.middle,
                               color.rgb.white])
    else:
        c.stroke(path.circle(x, y, r),
                 [bgcolor, deco.filled([color.rgb.white])])
        c.text(x, y, label, [bgcolor, text.halign.center, text.valign.middle])
