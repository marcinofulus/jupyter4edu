...
programming language for undegraduate students of ``Applied Computer
Science'' programme. There has been implemented a concept of ``mobile
computer classroom'' which consists of inexpensive and portable
thin-client computers which are available to students in the rental
office. They are used to access compute resources where the required
software is available.  Chromebooks currently play the role of thins
clients, which is why all services must be available through a web
browser. Jupyter notebook fits perfectly in such a scenario. In recent
years several teaching workflows based on Juyter notebook have been
tested and implemented.

At the Institute of Physics, the Jupyter notebook is not the first
teaching experience with web based software for mathamatical
explorations. SageMath\cite{sagemath}, the Python-based computer
algebra system has been served to students and academic teachers for
several years. The Sage notebook (\texttt{sagenb}) provided ubiquitous
access to computations in science education, and a large number of
teaching materials where produced. This technology was also replicated
in partner high schools as part of the iCSE4school project (details
see \cite{icse4school}). Since 2011, c.a. 2000 students and over 500
high school pupils have used the system. The Sage notebooks have been
served using a small computer cluster and until today users have
created over 30000 notebook documents.  Over time, the notebook system
\texttt{sagenb} has become obsolete and the currently SageMath
software is available as a kernel to the Jupyter notebook. Therefore
en effort was initiated to replace old \texttt{sagenb} notebooks
system for the sake of Jupyter, and to expand this technology to other
teaching programmes.

In this paper, we will the course: ``Introduction to Artificial
Intelligence``, which is attended by c.a. 50 Applied Computer Science
students every year. The course uses Jupyter notebook as a main
teaching tool during both the lectures and tutorials. All materials
are discributed and collected in as \texttt{*.ipynb} file. Moreover,
students submit their assignments \texttt{nbgrader} software installed
on publicly available web-address. All excercices include automated
tests to help both students in self assessment as well as teachers in
grading. 
...
\caption{Jupyter notebooks is a executable document which contains
  rich-text and code (e.g. in Python, R). }
\label{fig:notebook}
\end{figure}
...
\section{Creating and Grading assignments with \texttt{nbgrader}}
...
\section{Typical workflow with Jupyter and nbgrader}

\subsection{Subnotebooks }



\subsection{In Lecture hand out  Perpectron-quizz}

Jupyter notebook is used as hand-out material given during the lecture on learning Perceptron 
